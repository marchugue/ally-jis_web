...
export async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    // On 401: clear the stored token and signal AuthContext to sign out.
    // Only do this for authenticated requests (auth=true) to avoid firing on
    // public endpoints that intentionally return 401.
